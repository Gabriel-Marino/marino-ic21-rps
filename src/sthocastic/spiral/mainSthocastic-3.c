...
void op(int numSim, int t, int *phi) {
    ...
    sprintf(name, "dat/rps-spiral-%d-%d-%d.dat", Ns, t, numSim);
    file = fopen(name, "w");
    for (i = 0; i < Ni; i++) {
        for (j = 0; j < Nj; j++) {
...
int main(int argc, char **argv) {
    ...
    double  action, inv_emp /**inv_emp stand for inverse empty, will used to store the inverse of empty spots density**/, a = r;
    int i, j,                               //
        k = 0,                              //  k       is the Counter of generations given to the op function;
        l, t,                               //
        gd,                                 //  gd      stands for grid, used for locate a point inside the grid;
        act,                                //  act     stands for Active individual;
        pas,                                //  pas     stands for Passive individual;
        nebr,                               //  nebr    stands for Neighboor, used to ;
        temp,                               //  temp    stands for Temporary;
        ...
        *phi = calloc(Ni*Nj, sizeof(int));  //  phi is the pointer that locates each individual in the grid;
    char namedst[100], nameInvEmp[100], nameAct[100];

    gsl_rng_default_seed = (argc == 2) ? atoi(argv[1]) : time(NULL);
    gsl_rng *rng = gsl_rng_alloc(gsl_rng_taus);

    sprintf(namedst, "dat/dst/dst-spiral-%d-%d.dat", Ns, numSim);
    sprintf(nameInvEmp, "dat/emp/invEmp-spiral-%d-%d.dat", Ns, numSim);
    sprintf(nameAct, "dat/act/act-cont-spiral-%d-%d.dat", Ns, numSim);
...
//  Main Loop;
    for (t = 0; t < tf+1; t++) {
        ...
        while (gd < Ni*Nj) {    /**evalute if the point gd is inside the grid of size Ni*Nj, and if true he keep working 
                                    to ensure that every point will be update in every temporal step**/
            do {
