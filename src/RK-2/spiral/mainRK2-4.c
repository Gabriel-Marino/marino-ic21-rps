...
void op(int t, double *p1, double *p2, double *p3, double *p4, double *p0) {
    int i, j;
    FILE *file;
    char name[100];
    ...
    for (i = 0; i < Ni; i++) {
        for (j = 0; j < Nj; j++) {
            fprintf(file, "%e ", 1.0*p1[i*Nj+j]+2.0*p2[i*Nj+j]+3.0*p3[i*Nj+j]+4.0*p4[i*Nj+j]);
        };
        fprintf(file, "\n");
    };
    fclose(file);
...
//      First step of Runge-Kutta;
            for (i = 0; i < Ni; i++) {
                for (j = 0; j < Nj; j++) {
                    ...
                    p_1[l] = p1[l]+0.5*dt*(Cm*Laplaciano+Pr*p0[l]*p1[l]-Pp*p1[l]*(p4[l]+p3[l]));

                    Laplaciano = p2[lpx]+p2[lmx]+p2[lpy]+p2[lmy]-4.0*p2[l];
                    p_2[l] = p2[l]+0.5*dt*(Cm*Laplaciano+Pr*p0[l]*p2[l]-Pp*p2[l]*(p1[l]+p4[l]));

                    Laplaciano = p3[lpx]+p3[lmx]+p3[lpy]+p3[lmy]-4.0*p3[l];
                    p_3[l] = p3[l]+0.5*dt*(Cm*Laplaciano+Pr*p0[l]*p3[l]-Pp*p3[l]*(p2[l]+p1[l]));

                    Laplaciano = p4[lpx]+p4[lmx]+p4[lpy]+p4[lmy]-4.0*p4[l];
                    p_4[l] = p4[l]+0.5*dt*(Cm*Laplaciano+Pr*p0[l]*p4[l]-Pp*p4[l]*(p3[l]+p2[l]));

                    Laplaciano = p0[lpx]+p0[lmx]+p0[lpy]+p0[lmy]-4.0*p0[l];
                    p_0[l] = p0[l]+0.5*dt*(Cm*Laplaciano-Pr*p0[l]*(p1[l]+p2[l]+p3[l]+p4[l])+Pp*(p1[l]*(p4[l]+p3[l])+
                                                                                                p2[l]*(p1[l]+p4[l])+
                                                                                                p3[l]*(p2[l]+p1[l])+
                                                                                                p4[l]*(p3[l]+p2[l])));
                };
...
//      Second step of Runge-Kutta;
            for (i = 0; i < Ni; i++) {
                for (j = 0; j < Nj; j++) {
                    ...
                    p1[l] += 0.5*dt*(Cm*Laplaciano+Pr*p_0[l]*p_1[l]-Pp*p_1[l]*(p_4[l]+p_3[l]));

                    Laplaciano = p_2[lpx]+p_2[lmx]+p_2[lpy]+p_2[lmy]-4.0*p_2[l];
                    p2[l] += 0.5*dt*(Cm*Laplaciano+Pr*p_0[l]*p_2[l]-Pp*p_2[l]*(p_1[l]+p_4[l]));

                    Laplaciano = p_3[lpx]+p_3[lmx]+p_3[lpy]+p_3[lmy]-4.0*p_3[l];
                    p3[l] += 0.5*dt*(Cm*Laplaciano+Pr*p_0[l]*p_3[l]-Pp*p_3[l]*(p_2[l]+p_1[l]));

                    Laplaciano = p_4[lpx]+p_4[lmx]+p_4[lpy]+p_4[lmy]-4.0*p_4[l];
                    p4[l] += 0.5*dt*(Cm*Laplaciano+Pr*p_0[l]*p_4[l]-Pp*p_4[l]*(p_3[l]+p_2[l]));

                    Laplaciano = p_0[lpx]+p_0[lmx]+p_0[lpy]+p_0[lmy]-4.0*p_0[l];
                    p0[l] += 0.5*dt*(Cm*Laplaciano-Pr*p_0[l]*(p_1[l]+p_2[l]+p_3[l]+p_4[l])+Pp*(p_1[l]*(p_4[l]+p_3[l])+
                                                                                               p_2[l]*(p_1[l]+p_4[l])+
                                                                                               p_3[l]*(p_2[l]+p_1[l])+
                                                                                               p_4[l]*(p_3[l]+p_2[l])));
                };
